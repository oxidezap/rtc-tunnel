...
#
# The toolchain is never modified in place. `RUSTUP_HOME` and `CARGO_HOME` are
# redirected under `target/`, so `rm -rf target/rustup target/cargo-home`
# removes every trace. Point `RTC_TUNNEL_CARGO_HOME` at an existing cargo home to
# reuse its crate cache.
#
...
# Isolated, disposable homes. Nothing outside target/ is written.
export RUSTUP_HOME="${RTC_TUNNEL_RUSTUP_HOME:-$repo_root/target/rustup}"
...
# Install the pinned toolchain and its sources into the isolated home. This is
# a no-op once present.
rustup toolchain install "$toolchain" --profile minimal --component rust-src >/dev/null
