...
pub(crate) mod proto;
// `socket` wraps `socket2`, which does not target wasm. The module is a
// convenience builder for hosts; the mDNS protocol itself never uses it, so it
// is omitted where it cannot compile.
